...
vcf.fn <- "/home/tari/tomato/RAD2Core/gatk/GVCF/combined.filtered.snp.vcf.gz"
snpgdsVCF2GDS(vcf.fn, "/home/tari/tomato/RAD2Core/gatk/GVCF/combined.filtered.snp.gds", method="biallelic.only")
# Open the GDS file
genofile <- snpgdsOpen("/home/tari/tomato/RAD2Core/gatk/GVCF/combined.filtered.snp.gds")
# Summary
...
## Run PCA
pca <- snpgdsPCA(genofile, snp.id=snpset.id, num.thread=16, autosome.only=FALSE)
# variance proportion (%)
pc.percent <- pca$varprop*100
head(round(pc.percent, 2))
# Get population information
pop_code <- read.table("/home/tari/tomato/RAD2Core/gatk/info/cult.wild.pop.txt", header=FALSE) 
...
# Try ggplot
p <- ggplot(tab, aes(x=EV2, y=EV1, col=pop)) + geom_point(alpha=0.5) #+ geom_text(aes(label=tab$sample.id), hjust=0, vjust=0) 
p
...
ibs <- snpgdsIBS(genofile, num.thread=16, autosome.only=FALSE)
